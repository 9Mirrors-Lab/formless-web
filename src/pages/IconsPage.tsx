...
export default function IconsPage() {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.to('.neural-node', { scale: 1.3, duration: 1.2, yoyo: true, repeat: -1, stagger: { each: 0.2, from: 'random' }, ease: 'sine.inOut', transformOrigin: '50% 50%' });
      gsap.to('.neural-link', { opacity: 0.3, duration: 1.5, yoyo: true, repeat: -1, stagger: { each: 0.3, from: 'random' }, ease: 'power1.inOut' });
      gsap.to('.molecule-group', { rotation: 360, repeat: -1, ease: 'none', duration: 20, transformOrigin: '50% 50%' });
      gsap.to('.molecule-atom', { scale: 1.4, duration: 1.5, yoyo: true, repeat: -1, stagger: 0.4, ease: 'sine.inOut', transformOrigin: '50% 50%' });
      gsap.to('.quantum-ring-1', { rotation: 360, repeat: -1, duration: 8, ease: 'none', transformOrigin: '50% 50%' });
      gsap.to('.quantum-ring-2', { rotation: -360, repeat: -1, duration: 12, ease: 'none', transformOrigin: '50% 50%' });
      gsap.to('.quantum-ring-3', { rotation: 360, repeat: -1, duration: 16, ease: 'none', transformOrigin: '50% 50%' });
      gsap.to('.quantum-core-dot', { scale: 1.5, opacity: 0.7, duration: 1, yoyo: true, repeat: -1, ease: 'power1.inOut', transformOrigin: '50% 50%' });

      // --- 10 NEW PHILOSOPHICAL ICONS ---
      
      // 1. The Observer
      gsap.to('.observer-wave', { scale: 2.5, opacity: 0, duration: 2.5, repeat: -1, stagger: 0.6, ease: 'power1.out', transformOrigin: '50% 50%' });
      
      // 2. Creating Space
      gsap.to('.space-circle-left', { attr: { cx: 28 }, duration: 2.5, yoyo: true, repeat: -1, ease: 'power2.inOut' });
      gsap.to('.space-circle-right', { attr: { cx: 72 }, duration: 2.5, yoyo: true, repeat: -1, ease: 'power2.inOut' });
      
      // 3. The Awakening
      gsap.fromTo('.wake-sun', { y: 15 }, { y: -10, duration: 3, yoyo: true, repeat: -1, ease: 'power2.inOut' });
      gsap.fromTo('.wake-beam', { scaleY: 0, opacity: 0 }, { scaleY: 1, opacity: 1, duration: 1.5, yoyo: true, repeat: -1, stagger: 0.15, ease: 'power2.out', transformOrigin: 'bottom center', delay: 1.5 });
      
      // 4. Grounded
      gsap.to('.ground-wind', { rotation: 360, duration: 8, repeat: -1, ease: 'none', transformOrigin: '50% 50%' });
      gsap.fromTo('.ground-root', { strokeDasharray: "0 100" }, { strokeDasharray: "100 100", duration: 3, repeat: -1, yoyo: true, ease: 'power1.inOut' });
      
      // 5. Fog to Clarity
      const tlFog = gsap.timeline({ repeat: -1, yoyo: true, repeatDelay: 1 });
      tlFog.to('.fog-dot', { x: 0, y: 0, opacity: 1, duration: 2, stagger: { each: 0.05, from: "random" }, ease: 'power3.inOut' });
      
      // 6. Illusion of Problems
      gsap.to('.illusion-box', { opacity: 0, scale: 1.2, duration: 3, repeat: -1, yoyo: true, ease: 'power2.inOut', transformOrigin: '50% 50%' });
      gsap.fromTo('.illusion-ring', { scale: 0.8, opacity: 0 }, { scale: 1, opacity: 1, duration: 3, repeat: -1, yoyo: true, ease: 'power2.inOut', delay: 1.5, transformOrigin: '50% 50%' });
      
      // 7. The Undertow
      const tlUnder = gsap.timeline({ repeat: -1, repeatDelay: 1 });
      tlUnder.fromTo('.undertow-group', { rotation: 0 }, { rotation: 1080, duration: 4, ease: 'power3.out', transformOrigin: '50% 50%' });
      
      // 8. The Flow / Allowing
      gsap.to('.flow-wave', { x: -100, duration: 2.5, repeat: -1, ease: 'none' });
      
      // 9. Releasing Judgment
      gsap.to('.release-knot', { scale: 0.5, opacity: 0, duration: 2.5, repeat: -1, yoyo: true, ease: 'power2.inOut', transformOrigin: '50% 50%' });
      gsap.fromTo('.release-line', { y: 20, opacity: 0 }, { y: -15, opacity: 1, duration: 2, repeat: -1, yoyo: true, ease: 'power1.inOut', stagger: 0.2, delay: 1 });
      
      // 10. True North
      const tlNorth = gsap.timeline({ repeat: -1, repeatDelay: 2 });
      tlNorth.to('.north-needle', { rotation: 70, duration: 0.15, transformOrigin: '50% 50%', ease: 'power1.inOut' })
      ...
      tlVoice.to('.voice-ring', { rotation: 180, duration: 2, ease: 'power2.inOut', transformOrigin: '50% 50%' })
             .to('.voice-ring', { scale: 0.8, opacity: 0, duration: 1, ease: 'power1.in', transformOrigin: '50% 50%' }, "+=0.5")
             .to('.voice-ring', { scale: 1, opacity: 1, rotation: 0, duration: 1, transformOrigin: '50% 50%' }, "+=0.5");

      // 2. The Pause (Stillness)
      const tlPause = gsap.timeline({ repeat: -1, yoyo: true, repeatDelay: 1 });
      tlPause.to('.pause-bar-left', { x: 12, duration: 1.5, ease: 'power2.inOut' }, 0)
             .to('.pause-bar-right', { x: -12, duration: 1.5, ease: 'power2.inOut' }, 0)
             .to(['.pause-bar-left', '.pause-bar-right'], { scaleY: 0.2, duration: 1, ease: 'power2.inOut', transformOrigin: '50% 50%' }, 1.5);

      // 3. The Reflection
      const tlReflect = gsap.timeline({ repeat: -1, yoyo: true, repeatDelay: 1 });
      tlReflect.fromTo('.reflect-circle-left', { x: -20 }, { x: -5, duration: 2, ease: 'power2.inOut' }, 0)
      ...
      // 4. The Relief
      gsap.to('.relief-box', { attr: { rx: 25, ry: 25 }, rotation: 90, scale: 0.8, strokeWidth: 1, opacity: 0.5, duration: 2.5, repeat: -1, yoyo: true, ease: 'power1.inOut', transformOrigin: '50% 50%' });

      // 5. The Trigger
      const tlTrigger = gsap.timeline({ repeat: -1, repeatDelay: 1 });
      tlTrigger.to('.trigger-arrow', { x: 15, duration: 0.5, ease: 'power2.in' })
               .to('.trigger-shield', { x: 5, scaleX: 0.6, duration: 0.5, ease: 'power2.out', transformOrigin: 'center left' }, "<")
      ...
      tlShift.to('.shift-diamond', { rotation: 45, scale: 0.8, duration: 1.5, ease: 'power2.inOut', transformOrigin: '50% 50%' })
             .to('.shift-core', { scale: 3, opacity: 0.8, duration: 1.5, ease: 'power2.inOut', transformOrigin: '50% 50%' }, "<");

      // 7. The Tangle
      const tlTangle = gsap.timeline({ repeat: -1, yoyo: true, repeatDelay: 1 });
      tlTangle.to('.tangle-chaos', { opacity: 0, scale: 0.8, duration: 1.5, ease: 'power1.inOut', transformOrigin: '50% 50%' }, 0)
              .fromTo('.tangle-straight', { opacity: 0, scaleX: 0 }, { opacity: 1, scaleX: 1, duration: 1.5, ease: 'power2.inOut', transformOrigin: '50% 50%' }, 0.5);

      // 8. The Anchor — harmonic swing: pivot in SVG space at the string top (matches hinge dot)
      gsap.fromTo(
        '.anchor-pendulum',
      ...
      // 9. The Formless
      gsap.to('.formless-ring', { scale: 2, opacity: 0, duration: 4, repeat: -1, ease: 'power1.out', stagger: 1.3, transformOrigin: '50% 50%' });

      // --- Large-format loops (full-frame) ---
      // 1. The Rift
      const tlRift = gsap.timeline({ repeat: -1, yoyo: true, repeatDelay: 1 });
      tlRift.to('.rift-lines line', {
        x: (i, _target, targets) => {
      ...
      tlRift.to('.rift-core', { opacity: 0.9, scale: 1.5, duration: 3, ease: 'power2.inOut', transformOrigin: '50% 50%' }, 0);

      // 2. The Infinite Horizon
      gsap.fromTo('.horizon-ring', 
        { scale: 0.1, opacity: 0 },
        {
          keyframes: [
      ...
      gsap.to('.tangle-group', { rotation: 360, duration: 40, repeat: -1, ease: 'none', transformOrigin: '50% 50%' });
