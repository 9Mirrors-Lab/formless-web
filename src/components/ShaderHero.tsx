...
import { HeroBackdrop } from '@/components/shader/HeroBackdrop';
import type { BackgroundId } from '@/components/shader/backgroundOptions';
...
export function ShaderHero({ theme = 'forest' }: ShaderHeroProps) {
  const container = useRef<HTMLElement>(null);
  const { restricted } = useSiteAccess();
  const { getText, getLink } = useContent();

  const ctaEnabled = !restricted;
  ...
  return (
    <section
      ref={container}
      className="home-hero home-hero--elevated-copy relative w-full overflow-hidden"
    >
      <HeroBackdrop backgroundId={theme} position="absolute" />
      ...
      <div className="home-hero__stage relative z-10">
        <div className="shader-hero-content home-hero__container">
          <div className="home-hero__copy flex flex-col gap-2">
            <span className="shader-hero-elem mb-0 block font-mono text-[clamp(0.65rem,0.2vw+0.6rem,0.875rem)] uppercase tracking-[0.3em] text-cream/60 md:mb-4">
              {cx('eyebrow')}
            </span>
            <h1 className="home-hero__headline font-serif font-light italic leading-[1.08] tracking-tight text-cream">
              <span className="shader-hero-elem block">{cx('headline_primary')}</span>
              <span className="shader-hero-elem home-hero__headline-secondary block">
                {cx('headline_secondary')}
              </span>
            </h1>
            <p className="shader-hero-elem home-hero__lede mt-8 whitespace-pre-line font-sans font-light leading-relaxed text-cream/60">
              {lede}
            </p>
            {ctaEnabled && cta ? (
              <a
